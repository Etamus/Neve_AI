...
from neveai.retrieval.vector.utils import process_metadata
from neveai.retrieval.vector.main import (
    VectorDBBase,
    VectorItem,
    GetResult,
    SearchResult,
)
from neveai.config import S3_VECTOR_BUCKET_NAME, S3_VECTOR_REGION
...
class S3VectorClient(VectorDBBase):
    """
    AWS S3 Vector integration for Neve Knowledge.
    """
    ...
    def _filter_metadata(
        ...
        # Keep only the first 10 keys, prioritizing important ones based on actual Neve metadata
        important_keys = [
    ...
    def get(self, collection_name: str) -> Optional[GetResult]:
        ...
        try:
            ...
            # The Neve GetResult expects lists of lists, so we wrap each list
            if all_ids:
                return GetResult(
