...
def set_faster_whisper_model(model: str, auto_update: bool = False):
    ...
    if model:
        try:
            ...
            log.warning("faster_whisper not installed, local STT unavailable")
            return None
...
def load_speech_pipeline(request):
    try:
        ...
        log.warning("transformers/datasets not installed, speech pipeline unavailable")
        return
    ...
    if request.app.state.config.TTS_ENGINE == "openai":
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=detail if detail else "Neve: Server Connection Error",
            )
