<script lang="ts">
	...
	/**
	 * Normalize code fence language: extract just the language name.
	 * Some LLMs output ```html<html> (no newline), making lang="html<html>".
	 * This extracts just "html" by trimming at the first non-alphanumeric char
	 * (except - + # which are valid in lang names like c++, c#, objective-c).
	 */
	...
	/**
	 * Fix code content from malformed LLM fences.
	 * 1. If lang has overflow (e.g. "html<html>"), prepend the overflow to code
	 *    since marked consumed it as part of the language string.
	 * 2. Strip trailing ``` that is a malformed closing fence
	 *    (LLM put closing fence on the same line as code, not its own line).
	 */
	const normalizeCode = (lang: string, code: string): string => {
		// 1. Extract overflow from original lang and prepend to code
		// e.g. lang="html<html>" → overflow="<html>", prepend with newline
		if (lang) {
			const match = lang.match(/^[\w+#.-]+/);
			if (match && match[0].length < lang.length) {
				const overflow = lang.slice(match[0].length);
				code = overflow + '\n' + code;
			}
		}
		// 2. Strip trailing ``` that is a malformed closing fence
		// Only when preceded by non-backtick content on the same line
		code = code.replace(/([^`\n])```\s*$/, '$1');
		return code;
	};
