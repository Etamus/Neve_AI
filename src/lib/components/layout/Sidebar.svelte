<script lang="ts">
	...
	const initChatList = async () => {
		...
		// Run all three API calls in parallel with individual error handling
		await Promise.all([
			(async () => {
				try {
	...
	onMount(() => {
		// Clear any stale collapsed state for the sidebar-chats folder
		localStorage.removeItem('sidebar-chats-folder-state');
		...
		// Always pre-load chat list data regardless of sidebar visibility
		if (!$showSidebar && localStorage.token) {
			initChatList().catch((e) => console.error('Failed to pre-load chat list:', e));
		}
			...
			<div>
				<div class=" py-2 flex justify-center items-center">
					{#if $user !== undefined && $user !== null}
						...
						>
							<div
								class=" cursor-pointer flex rounded-md hover:bg-gray-100 dark:hover:bg-gray-700/50 transition group"
							>
			...
			<div class="px-1.5 pt-1.5 pb-2 sticky bottom-0 z-10 -mt-3 sidebar">
				...
				<div class="flex flex-col">
					{#if $user !== undefined && $user !== null}
						...
						>
							<div
								class=" flex items-center rounded-md py-2 px-1.5 w-full hover:bg-gray-100 dark:hover:bg-gray-700/50 transition"
							>
